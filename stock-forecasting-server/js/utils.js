function dropout_nn(x, keep_prob) {
  // Generate a tensor of uniform random values with the same shape as x
  uniform = tf.randomUniform(x.shape)

  // Add keep_prob to the uniform random tensor
  added = tf.add(tf.scalar(keep_prob), uniform)

  // Perform element-wise floor operation on the added tensor
  binary = tf.floor(added)

  // Scale x by keep_prob and set each element to zero with probability 1-keep_prob
...
function dropout_lstm(cell, a, h, c, dropout_input = 1.0, dropout_output = 1.0) {
  ...
  // Return concatenated outputs, updated hidden and cell states
...
// Async function to call the sleep function and wait for it to resolve
async function async_sleep(ms) {
  // Wait for the sleep promise to resolve before continuing
  await sleep(ms);
}

// Function to waste time by doing nothing in a loop
function wasting_time(count) {
  // Loop from 0 to count (exclusive)
  for (var n = 0; n < count; n++) {
    // Do nothing in the loop body
  }
}
function arange(start, end, skip) { // function to generate range
  var arr = [start]  // initialize array with start
  while ((parseFloat(arr.slice(-1)) + parseFloat(skip)) < (end))  // while last element + skip < end
    arr.push(parseFloat(arr.slice(-1)) + parseFloat(skip))  // add skip to last element and add to array
  return arr  // return the range array
}
// Convert TensorFlow tensor to list
async function tf_tolist(a) {
  // Initialize empty array to hold tensor data
  var arr = []

  // Iterate over tensor's first dimension
  for (var i = 0; i < a.shape[0]; i++) {
    // Slice tensor data and convert to JavaScript array
    var tensorData = await a.slice([0, 0], [1, -1]).data();
    // Add sliced data to arr as new sub-array
    arr.push(Array.prototype.slice.call(tensorData));
  }

  // Return final array
  return arr
}
// convert tensorflow string to list
function tf_str_tolist(a) {
  // remove "tensorflow:", convert to string, and trim
  var str = a.toString().slice(7).trim();
  // parse string to json and return list
  return JSON.parse(str);
}
// Convert TensorFlow.js tensor to JavaScript array
function tf_slice_tolist(a) {
  // Initialize empty array to hold tensor values
  var arr = []

  // Iterate over tensor's first dimension
  for (var i = 0; i < a.shape[0]; i++) {
    // Slice tensor at current index and convert to string
    var tensor_str = a.slice([i, 0], [1, 1]).toString()

    // Remove first 7 characters and replace comma with empty string
    var val = JSON.parse(tensor_str.slice(7).trim().replace(',', ''))[0][0]

    // Add value to array
    arr.push(val);
  }

  // Return resulting JavaScript array
  return arr
}
function tf_nj_list(a) {
  ...
  // Iterate over each element in input array 'a'
  for (var i = 0; i < a.shape[0]; i++) {
    for (var k = 0; k < a.shape[1]; k++) {
      // Parse JSON string to get the value
      arr[i][k] = JSON.parse(a.slice([i, k], [1, 1]).toString().slice(7).trim().replace(',', ''))[0][0]
    }
  }
...
function tf_nj_list_flatten(a) {
  // Initialize result array with correct size
...
function label_encoder(arr) { // Function to encode labels
  var unique = [...new Set(arr)]; // Store unique values in array
  var encoder = []
  for (var i = 0; i < arr.length; i++) // Loop through input array
    encoder.push(unique.indexOf(arr[i])) // Encode value to index of unique array
  return { 'unique': unique, 'encode': encoder } // Return unique values and encoded array
}
function shuffle(arr1, arr2) {
  ...
  while (index) {
    // get random number up to index
    rnd = Math.floor(Math.random() * index);
    // decrease index
    index -= 1;
    // save values at index
    tmp1 = arr1[index];
    tmp2 = arr2[index];
    // swap values at index and random position
    arr1[index] = arr1[rnd];
    arr2[index] = arr2[rnd];
    arr1[rnd] = tmp1;
    arr2[rnd] = tmp2;
...
function get_index(arr, val) { // get indices of val in arr
  var indices = []          // create empty array for indices
  for (var i = 0; i < arr.length; i++) // loop through arr
    if (arr[i] == val)      // if current element equals val
      indices.push(i)      // add index to indices array
  return indices           // return indices array
}
function get_elements(arr, indices) {
  // Initialize empty array to store elements
  var elements = []

  // Iterate over indices
  for (i in indices) {
    // Get element at current index and push to elements array
...
function svd(a, n_components) {
  ...
  output_svd = numer.svd()

  // Convert U to TensorFlow tensor
  tensor_U = tf.tensor(output_svd['U'])

  // Slice V to desired number of components
...
function nnmf(arr, n_components) {
  ...
  for (var i = 0; i < 100; i++) {
    ...
    // Minimize cost using optimizer
    optimizer.minimize(() => cost(f(), a));
  }

  // Return W as a nested JavaScript array
...
function metrics(a) {
  ...
  // Return variance and standard deviation as object
  return { 'std': std, 'variance': variance }
}
// Function to perform standard scaling on tensor 'a'
function standard_scaling(a) {
  // Subtract mean from 'a' and square the result
  squared = tf.square(tf.sub(a, a.mean(0))).sum(0)
  ...
  return tf.div(tf.sub(a, a.mean(0)), std) // standardize values
}
// Function to perform min-max scaling on a tensor
function minmax_scaling(a) {
  // Convert input to tensor
  a = tf.tensor(a)

  // Subtract minimum value of tensor from each element
  a = tf.sub(a, a.min(0))

  // Subtract minimum value of tensor from maximum value of tensor
  const max_minus_min = tf.sub(a.max(0), a.min(0))

  // Divide each element by the range of values
  return tf.div(a, max_minus_min)
}
// Function to scale 1D array between 0 and 1
function minmax_1d(a) {
  // Convert input to tensor
  a = tf.tensor(a)

  // Compute minimum value
  a_min = tf_str_tolist(a.min()) // Convert to list

  // Compute maximum value
  a_max = tf_str_tolist(a.max()) // Convert to list

  // Scale values between 0 and 1
  scaled = tf.div(tf.sub(a, a.min()), tf.sub(a.max(), a.min()))

  // Return scaled values and min/max
  return { 'scaled': scaled, 'min': a_min, 'max': a_max }
}
function reverse_minmax_1d(a, a_min, a_max) {
  // Scale and shift input tensor a to new range
  return tf.add(tf.mul(a, tf.scalar(a_max - a_min)), tf.scalar(a_min))
}
function one_hot(label_encoder) {    // Function to convert labels to one-hot encoding
  var onehot = nj.zeros([      // Initialize a 2D array of zeros
    label_encoder['encode'].length,    // Number of labels
    label_encoder['unique'].length     // Number of unique labels
  ]).tolist();

  for (var i = 0; i < label_encoder['encode'].length; i++) {   // Iterate through labels
    onehot[i][label_encoder['encode'][i]] = 1;    // Set the corresponding index to 1
  }

  return onehot;    // Return the one-hot encoded labels
}
...
function kernelDensityEstimator(kernel, x) {
  // Return a function that calculates density estimates
  return function (sample) {
    // Map over x values and calculate density estimate for each
    return x.map(function (x) {
      // Calculate density estimate using the kernel function
      return [x, d3.mean(sample, function (v) {
        // Subtract sample value from x value and apply kernel function
        return kernel(x - v);
      })];
    });
  };
}
// Epanechnikov Kernel function with bandwidth
function epanechnikovKernel(bandwith) {
  // Return an anonymous function
  return function (u) {
    // Calculate absolute value of u/bandwidth
    if (Math.abs(u = u / bandwith) <= 1) {
      // Return the kernel value
      return 0.75 * (1 - u * u) / bandwith;
      // Return 0 if u is not within bandwidth
    } else return 0;
  };
}
function histogram(arr, bins = 30, norm = true, density = false, jitter = 0.001) {
  var max_arr = Math.max(...arr) // Find max value in arr
  var min_arr = Math.min(...arr) // Find min value in arr
  var arr_bins = []        // Initialize arr_bins
  var start = min_arr     // Initialize start
  var skip = (max_arr - min_arr) / bins    // Calculate skip size
  var x_arange = []     // Initialize x_arange
  while (arr_bins.length < bins) {    // Loop to create bins
