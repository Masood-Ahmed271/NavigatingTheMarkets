...
access_token = "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"
finnhub_client = finnhub.Client(
    api_key="cnjhg0hr01qmfbtbcur0cnjhg0hr01qmfbtbcurg")
...
model = PeftModel.from_pretrained(
    ...
    offload_folder="offload/",
    disable_model_loading_ram_optimization=True
)
model = model.eval()
...
def get_stock_data(stock_symbol, steps):
    ...
    if len(stock_data) == 0:
        raise gr.Error(f"""Failed to download stock price data for symbol {
                       stock_symbol} from yfinance!""")
...
def get_all_prompts_online(symbol, data, curday, with_basics=True):
    ...
    prompt = info + f"\n\nBased on all the information before {curday}, let's first analyze the positive developments and potential concerns for {symbol}. Come up with 2-4 most important factors respectively and keep them concise. Most factors should be inferred from company related news. " \
        f"""Then make your prediction of the {symbol} stock price movement for next week ({
        period}). Provide a summary analysis to support your prediction."""
